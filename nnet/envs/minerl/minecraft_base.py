...
class MinecraftBase:
  ...
  def __init__(
    ...
    # Make env
    with self._LOCK:
      from .import minerl_env
      self._gymenv = minerl_env.MineRLEnv(img_size, break_speed, gamma).make()
    ...
    # Actions
    self._noop_action = minerl_env.NOOP_ACTION
  ...
  def obs_space(self):
    return {
        ...
        'inventory': Space(np.float32, len(self._inv_keys), 0),
        'inventory_max': Space(np.float32, len(self._inv_keys), 0),
        'equipped': Space(np.float32, len(self._equip_enum), 0, 1),
        'reward': Space(np.float32),
        'health': Space(np.float32),
        'hunger': Space(np.float32),
        'breath': Space(np.float32),
        'is_first': Space(bool),
        'is_last': Space(bool),
        'is_terminal': Space(bool),
        **{f'log_{k}': Space(np.int64) for k in self._inv_keys},
        'log_player_pos': Space(np.float32, 3),
    }
  
  @property
  def act_space(self):
    return {
        'action': Space(np.int64, (), 0, len(self._action_values)),
        'reset': Space(bool),
    }
